...
def check_name(name):
    if (name.endswith(".txt")):
        ...
        print("Имя файла должно заканчиваться на '.txt'!")
    return None
...
def check_size(size):
    try:
        ...
        if (size > 0):
            ...
            print("Размер файла должен быть положительным!")
    except ValueError:
...
def main():
    ...
    if args.name:
        ...
        print("Введите имя файла (расширение - .txt):")
        while(True):
            name = check_name(input())
            if(name):
                break
        print("Введите размер файла (Mb):")
        while(True):
